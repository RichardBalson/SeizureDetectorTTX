function [h,a]=firls1(N,F,M,W,ftype)
...
%   $Revision: 1.11.4.8 $  $Date: 2010/02/17 19:00:09 $
...
if (ftype == 0),  % Type I and Type II linear phase FIR
    ...
    for s=1:2:length(F),
        ...
        b = b+(m/(4*pi*pi)*(cos(2*pi*k*F(s+1))-cos(2*pi*k*F(s)))./(k.*k))...
            * abs(W((s+1)/2)^2);
        b = b + (F(s+1)*(m*F(s+1)+b1)*sinc1(2*k*F(s+1)) ...
            - F(s)*(m*F(s)+b1)*sinc1(2*k*F(s))) ...
            * abs(W((s+1)/2)^2);
        if need_matrix
            G = G + (.5*F(s+1)*(sinc1(2*I1*F(s+1))+sinc1(2*I2*F(s+1))) ...
                - .5*F(s)*(sinc1(2*I1*F(s))+sinc1(2*I2*F(s))) ) ...
                * abs(W((s+1)/2)^2);
        end
    end;
    if Nodd
...
elseif (ftype == 1),  % Type III and Type IV linear phase FIR
    ...
    for s=1:2:length(F),
        if (do_weight((s+1)/2)),      % weight bands with 1/f^2
            ...
            b=b + ( m*snint1 ...
                + b1*2*pi*k.*( -sinc1(2*k*F(s+1)) + sinc1(2*k*F(s)) + csint1 ))...
                * abs(W((s+1)/2)^2);
