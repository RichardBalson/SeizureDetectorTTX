...
function Analyse_data_plot_multiple_background(data,window_length,frequency_bands,sampling_frequency,Animal_number,j,interleave,Date)
...
for m = 1:size(data,2)
    ...
    WriteExcel(data(:,m),Amplitude, Crossings,Crossing_time,Line_length,Animal_number,j,m,Date,t)
...
function WriteExcel(data,Amplitude, Crossings,Crossing_time,Line_length,k,j,m,Date,t)

[Year,Month,Day,Hours,Minutes,Seconds] = datevec(Date, 'dd/mm/yyyy HH:MM:SS');
Spreadsheet_Name = ['Animal ',int2str(k),' D ',int2str(Day),'_',int2str(Month),'_',int2str(Year),'.xls'];
Sheet_name = ['Seizure',int2str(j)];
column = [char((m-1)*6+65),'2'];
columnN = [char((m-1)*6+65),'1'];
columnD = [char((m-1)*6+65),'3'];
% columnE = [char((m-1)*5+68),'3'];
