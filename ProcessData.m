...
for Sheet =1:length(ExcelSheets)
    ...
    for k = 1:length(Sheetnames)
        if strcmp(Sheetnames{k}(1:5),'Seizu')
            ...
            for  j=1:Channels
                Padding(Seizure_number,j,:,:) = [mean(Data(1:PaddingIndex,AmplitudeInd(j)))... % 1ST DIMENSION AMPLITUDE, 2ND ZERO CROSSINGS 3RD LINE LENGTH, 4th mean, min, max, std
                                                   mean(Data(1:PaddingIndex,ZeroInd(j)))...
                                                   mean(Data(1:PaddingIndex,LLInd(j)));...
                                                   min(Data(1:PaddingIndex,AmplitudeInd(j)))... % 1ST DIMENSION AMPLITUDE, 2ND ZERO CROSSINGS 3RD LINE LENGTH
                                                   min(Data(1:PaddingIndex,ZeroInd(j)))...
                                                   min(Data(1:PaddingIndex,LLInd(j)));...
                                                   max(Data(1:PaddingIndex,AmplitudeInd(j)))... % 1ST DIMENSION AMPLITUDE, 2ND ZERO CROSSINGS 3RD LINE LENGTH
                                                   max(Data(1:PaddingIndex,ZeroInd(j)))...
                                                   max(Data(1:PaddingIndex,LLInd(j)));...
                                                   std(Data(1:PaddingIndex,AmplitudeInd(j)))... % 1ST DIMENSION AMPLITUDE, 2ND ZERO CROSSINGS 3RD LINE LENGTH
                                                   std(Data(1:PaddingIndex,ZeroInd(j)))...
                                                   std(Data(1:PaddingIndex,LLInd(j)))]';
