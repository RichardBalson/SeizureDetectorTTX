...
% description
% ~~~~~~~~~~~
% This script filters and extracts profusion data
...
function [Filtered_data dataIn] = Profusion_Ext_Filt(StartTime, Duration, Decimate, band_coeff,j)


try
    [~, DataChDec, ~] = Get_Data_ProFusionEEG4(j,StartTime-Time_adjustment,Duration,Decimate,0); % Extract data from profusion
catch
    [~, DataChDec, ~] = Get_Data_ProFusionEEG4A(j,StartTime-Time_adjustment,Duration,Decimate,0); % Extract data from profusion
end

k = min(find(DataChDec(:,1)));

dataIn = DataChDec(k:length(DataChDec),1);
...
% filter_data = filter(band_coeff,1,data);
...
function f_meas_output = freq_band_power(data,Fs,bands)
